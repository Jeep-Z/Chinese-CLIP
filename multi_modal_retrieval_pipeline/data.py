...
class EvalTxtDataset(Dataset):
    def __init__(self, jsonl_filename, max_txt_length=24):
        ...
        with open(jsonl_filename, "r", encoding="utf-8") as fin:
            for line in fin:
                ...
                self.texts.append((query_id, query_text))
    ...
    def __getitem__(self, idx):
        query_id, query_text = self.texts[idx]
        query_text = tokenize([_preprocess_text(str(query_text))], context_length=self.max_txt_length)[0]
...
def fetch_resolution(vision_model):
    ...
    vision_model_config_file = Path(__file__).parent.parent / f"cn_clip/clip/model_configs/{vision_model.replace('/', '-')}.json"
