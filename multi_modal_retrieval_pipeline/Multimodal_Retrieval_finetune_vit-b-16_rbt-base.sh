...
# data options
train_data=${DATAPATH}/datasets/Multimodal_Retrieval/lmdb/train
val_data=${DATAPATH}/datasets/Multimodal_Retrieval/lmdb/valid # if val_data is not specified, the validation will be automatically disabled
...
name=Multimodal_Retrieval_finetune_vit-b-16_roberta-base_bs128_1gpu
...
python3 -m torch.distributed.launch --use_env --nproc_per_node=${GPUS_PER_NODE} --nnodes=${WORKER_CNT} --node_rank=${RANK} \
          --master_addr=${MASTER_ADDR} --master_port=${MASTER_PORT} multi_modal_retrieval_pipeline/main.py \
