...
if __name__ == "__main__":
    ...
    vision_model_config_file = (
        ...
        / f"cn_clip/clip/model_configs/{args.vision_model.replace('/', '-')}.json"
    )
    ...
    text_model_config_file = (
        Path(__file__).parent.parent
        / f"cn_clip/clip/model_configs/{args.text_model.replace('/', '-')}.json"
    )
    ...
    assert os.path.exists(args.resume), "The checkpoint file {} not exists!".format(
        args.resume
    )
        ...
        with open(args.text_feat_output_path, "w") as fout:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    ...
                    for query_id, text_feature in zip(
                        query_ids.tolist(), text_features.tolist()
                    ):
                        fout.write(
                            "{}\n".format(
                                json.dumps(
                                    {"query_id": query_id, "feature": text_feature}
                                )
                            )
                        )
                        write_cnt += 1
        print(
            "{} text features are stored in {}".format(
                write_cnt, args.text_feat_output_path
            )
        )
    ...
    if args.extract_image_feats:
        ...
        if args.image_feat_output_path is None:
            ...
            args.image_feat_output_path = "{}.img_feat.jsonl".format(
                args.text_data.replace("_queries.jsonl", "_imgs")
            )
        write_cnt = 0
        with open(args.image_feat_output_path, "w") as fout:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    item_ids, images = batch
                    images = images.cuda(args.gpu, non_blocking=True)
                    image_features = model(images, None)
                    image_features /= image_features.norm(dim=-1, keepdim=True)
                    for item_id, image_feature in zip(
                        item_ids.tolist(), image_features.tolist()
                    ):
                        fout.write(
                            "{}\n".format(
                                json.dumps(
                                    {"item_id": item_id, "feature": image_feature}
                                )
                            )
                        )
                        write_cnt += 1
        print(
            "{} image features are stored in {}".format(
                write_cnt, args.image_feat_output_path
            )
        )
